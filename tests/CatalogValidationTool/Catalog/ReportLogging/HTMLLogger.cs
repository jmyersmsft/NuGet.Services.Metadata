...
namespace CatalogTestTool
{
    public class HTMLLogger
    {
        ...
        internal void WriteTable(Dictionary<string, List<string>> tableValues)
        {
...
function Toggle(arg1,arg2) {
	...
	if (el.style.display == ""block"") {
        ...
        private void AddHtmlTable(Dictionary<string, List<string>> tableValues)
        {
            htmlWriter.AddAttribute(HtmlTextWriterAttribute.Border, "1");
            htmlWriter.RenderBeginTag(HtmlTextWriterTag.Table);


            foreach (KeyValuePair<string, List<string>> keyValuePair in tableValues)
            {
                ...
                foreach (string error in keyValuePair.Value)
                {
                    htmlWriter.RenderBeginTag(HtmlTextWriterTag.Td);
                    htmlWriter.Write(error.ToString());
                    htmlWriter.RenderEndTag();
                }
