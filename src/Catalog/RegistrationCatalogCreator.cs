            ...
            int packageCountThreshold)
        {
            ...
            try
            {
                ...
                foreach (var graph in sortedGraphs)
                {
                    ...
                    //Remove ID and get just the version
                    existingId = existingId.Replace(id.ToLowerInvariant(), string.Empty);
                    string existingVersion = existingId.Substring(existingId.LastIndexOf("/") + 2);
