...
namespace Ng
{
    public static class Catalog2RegistrationDelete
    {
        static async Task Loop(string source, StorageFactory storageFactory, string contentBaseAddress, bool verbose, int interval)
        {
            CommitCollector collector = new RegistrationCatalogDeleteCollector(new Uri(source), storageFactory, CommandHelpers.GetHttpMessageHandlerFactory(verbose))
            {
                  ContentBaseAddress = new Uri(contentBaseAddress)
            };

            Storage storage = storageFactory.Create();
            ReadWriteCursor front = new DurableCursor(storage.ResolveUri("DeleteCursor.json"), storage, MemoryCursor.Min.Value);
